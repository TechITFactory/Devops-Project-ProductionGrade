# Sprint 3, Story 7.1: Install Prometheus Stack
...
### Step 1.1: Configure Default gp3 StorageClass
EKS requires a default StorageClass for the Prometheus StatefulSets to provision EBS volumes successfully.
...
# Remove default from old gp2 if it exists
kubectl patch sc gp2 -p '{"metadata": {"annotations":{"storageclass.kubernetes.io/is-default-class":"false"}}}'
```

### Step 1.2: Add Helm Repository
```bash
helm repo add prometheus-community https://prometheus-community.github.io/helm-charts
helm repo update
```

### Step 1.3: Create Namespace
```bash
kubectl create namespace monitoring
```

### Step 1.4: Create Grafana Admin Secret

**Never put passwords in the Helm values file** — they end up in Git. Instead, create a Kubernetes Secret first:
...
### Step 1.5: Create Values File
```bash
cat > prometheus-values.yaml << 'EOF'
# Prometheus Stack Configuration

# Grafana — credentials come from a Kubernetes Secret, NOT hardcoded here
grafana:
  enabled: true
  admin:
...
### Step 1.6: Install Stack

> **Note:** In production we install via ArgoCD (`apps/monitoring.yaml`). The `helm install` below is for manual/learning installs.
...
// collectDefaultMetrics() takes no arguments in prom-client v15+
// The { timeout } option was removed — just call it with no args
...
## Next: Story 7.2 (Grafana Dashboards)
