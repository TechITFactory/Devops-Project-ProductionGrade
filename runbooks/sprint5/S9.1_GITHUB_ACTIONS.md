...
### Key Features
1. **Change Detection**: Only builds services that changed (`dorny/paths-filter@v3`)
2. **Matrix Builds**: Parallel builds for changed services
3. **Single File**: One workflow for all 6 services
4. **Enforced quality gates**: lint and tests must pass before build
5. **Trivy security scan**: blocks on CRITICAL/HIGH fixable vulns (`exit-code: 1`, `ignore-unfixed: true`)
...
| `GITOPS_TOKEN` | Fine-grained PAT scoped to `techitfactory-gitops` repo only |
| `SONAR_TOKEN` | (Optional) SonarCloud |

> **GITOPS_TOKEN best practice:** Use a **fine-grained PAT** (not classic) scoped to only the `techitfactory-gitops` repository with `contents: write` permission. This limits blast radius if the token is compromised.
...
- [ ] Lint/tests run and block build on failure
- [ ] Trivy scan blocks on fixable CRITICAL/HIGH vulnerabilities
- [ ] Images pushed to ECR with commit SHA tag
- [ ] GitOps repo updated with new SHA tag
- [ ] ArgoCD syncs automatically
- [ ] Production release requires GitHub environment approval
