...
verify-deploy:
  ...
  steps:
    ...
    - name: Wait for ArgoCD to sync and deployment to roll out
      # ArgoCD auto-sync polls every 3 minutes — kubectl rollout status will
      # block until the new pods are running (up to 5 minutes total)
      run: |
        kubectl rollout status deployment/${{ matrix.service }} \
          -n techitfactory --timeout=300s

    - name: Verify health endpoint
      run: |
        PORT=3000
        # order-service runs on port 8000 (Python/FastAPI)
        [ "${{ matrix.service }}" == "order-service" ] && PORT=8000
        POD=$(kubectl get pods -n techitfactory -l app=${{ matrix.service }} \
          -o jsonpath='{.items[0].metadata.name}')
        kubectl exec -n techitfactory $POD -- \
          wget -q -O- http://localhost:${PORT}/health
```

> **Note:** ArgoCD auto-sync detects GitOps changes every 3 minutes (the default
> polling interval). If you need faster deploys during demos, trigger a manual
> sync: `argocd app sync $SERVICE_NAME`
